...
export async function register_admin_controller(req, res) {
    try {
        ...
        // Create credential for the admin user
        const credential = await create_credential_service(created_admin.id, user.password);
        
        if (!credential) {
            throw new VerificationError(400, "Failed to create admin credentials.");
        }
